"""Evaluate flow detectors on frozen CICIDS2017 multi-attack files.

Mirrors scripts/evaluate_cse_cic_ids2018.py procedure with the CICIDS2017
adapter; no detector, threshold, or methodology changes. Labels are read
AFTER detection for reporting only. Never persists incidents, never tunes
thresholds, never feeds labels to detectors.

Usage:
    backend\\.venv\\Scripts\\python.exe scripts/evaluate_cicids2017.py --flow005-full
"""
...
from app.services.datasets.cicids2017 import ADAPTER_VERSION, Cicids2017Adapter  # noqa: E402
...
DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "public" / "cicids2017" / "raw"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate flow detectors on CICIDS2017.")
    parser.add_argument("--file", action="append", default=[],
                        help="CSV filename (resolved under data/) or path. Repeatable.")
    parser.add_argument("--limit", type=int, default=0, help="Max rows per file (0 = unlimited).")
    ...
    parser.add_argument("--outdir", default="evaluation/cicids2017-multi-attack")
    args = parser.parse_args()

    limit = None if args.limit == 0 else args.limit
    files = [str(resolve_file(f)) for f in (args.file or list(DEFAULT_FILES))]
    ...
    for path_str in files:
        ...
        print(f"[progress] file-start {source_file}", flush=True)
        if chunked:
            ...
            print(f"[progress] file-done {source_file} detections={len(dets)}", flush=True)
        else:
    ...
    if chunked:
        ...
        print(f"[progress] label-pass wanted={len(wanted)}", flush=True)
        ...
        for path_str in files:
            ...
            labels.update(got)
            totals.update(tot)
            per_file_totals[path.name] = dict(tot)
        ...
        per_file_totals = {"-": dict(label_totals)}
    ...
    print(f"[progress] metrics detections={len(all_dets)}", flush=True)
    ...
    print(f"[progress] correlated incidents={len(incidents)}", flush=True)
    ...
    (outdir / "detection_metrics.json").write_text(
        json.dumps({"per_rule": per_rule, "overall": overall}, indent=2, default=str),
        encoding="utf-8")
    (outdir / "bucket_metrics.json").write_text(
        json.dumps({"bucket": bucket, "per_rule_bucket": per_rule_bucket,
                    "attack_labels": attack_labels}, indent=2, default=str),
        encoding="utf-8")
    (outdir / "incident_metrics.json").write_text(
        json.dumps(incident_summary, indent=2, default=str), encoding="utf-8")
    (outdir / "per_file_totals.json").write_text(
        json.dumps(per_file_totals, indent=2, default=str), encoding="utf-8")

    (outdir / "summary.json").write_text(json.dumps(summary, indent=2, default=str), encoding="utf-8")
    report = render_markdown(
        summary,
        title="CICIDS2017 Multi-Attack Flow Detection Evaluation",
        bucket_note="Attack episodes are defined per distinct non-benign label; "
                    "episode counts therefore reflect the frozen files' attack diversity.")
